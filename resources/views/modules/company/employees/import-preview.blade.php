<x-company-layout title="Revisar carga">
    <x-slot:actions>
        <form method="POST" action="{{ route('company.employees.import.cancel') }}">
            @csrf
            <x-ui.button type="submit" variant="secondary" size="sm">Cancelar</x-ui.button>
        </form>
        @if (($preview['error'] ?? 0) === 0)
            <form method="POST" action="{{ route('company.employees.import.commit') }}">
                @csrf
    ...
    <div
        ...
        x-data="{ filter: 'all' }"
    >
        <p class="text-sm text-slate-400">Revisión temporal. No se ha guardado nada. Corrige el Excel y vuelve a cargar si hay errores.</p>
        ...
        <div class="flex flex-wrap gap-2">
            <button type="button" class="rounded-lg px-3 py-1.5 text-xs" :class="filter === 'all' ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-300'" x-on:click="filter = 'all'">Todas</button>
            <button type="button" class="rounded-lg px-3 py-1.5 text-xs" :class="filter === 'ok' ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-300'" x-on:click="filter = 'ok'">Válidas</button>
            <button type="button" class="rounded-lg px-3 py-1.5 text-xs" :class="filter === 'error' ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-300'" x-on:click="filter = 'error'">Errores</button>
        </div>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-slate-900/80 text-slate-400 text-xs uppercase">
                    <tr>
                        ...
                        <th class="px-3 py-3 text-left">Cargo</th>
                ...
                <tbody class="divide-y divide-slate-800">
                    @foreach ($preview['rows'] as $row)
                        ...
                        >
                            ...
                            <td class="px-3 py-3 text-slate-400">{{ $row['job_title'] ?: '—' }}</td>
                            <td class="px-3 py-3 text-slate-400">{{ $row['email'] ?: '—' }}</td>
