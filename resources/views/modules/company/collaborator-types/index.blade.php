<x-company-layout title="Tipos de colaborador">
    @include('modules.company.settings.partials.nav-slots', ['companyNavActive' => 'tipos'])

    <div class="grid lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 rounded-xl border border-slate-800 bg-slate-900 overflow-hidden">
            <div class="px-4 py-3 border-b border-slate-800">
                <h3 class="text-sm font-semibold text-white">Catálogo de tipos</h3>
                <p class="text-xs text-slate-500 mt-1">Propios de esta empresa. El Excel crea el tipo si aún no existe.</p>
            </div>
            <div class="divide-y divide-slate-800">
                @forelse ($collaboratorTypes as $type)
                    <div x-data="{ editing: false }" class="px-4 py-3">
                        <div x-show="!editing" class="flex flex-wrap items-center justify-between gap-3">
                            <div class="flex flex-wrap items-center gap-3 text-sm">
                                ...
                                <span class="text-xs text-slate-500">{{ $type->employees_count }} empleado{{ $type->employees_count === 1 ? '' : 's' }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <button type="button" @click="editing = true" class="text-xs text-indigo-300 hover:text-indigo-200">Editar</button>
                                <form method="POST" action="{{ route('company.collaborator-types.destroy', $type) }}" onsubmit="return confirm('¿Eliminar este tipo?')">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <form x-show="editing" x-cloak method="POST" action="{{ route('company.collaborator-types.update', $type) }}" class="space-y-3">
                            @csrf
                            @method('PUT')
                            <div>
        ...
        <div class="rounded-xl border border-slate-800 bg-slate-900 p-4 h-fit">
            ...
            <form method="POST" action="{{ route('company.collaborator-types.store') }}" class="space-y-3">
                @csrf
                <div>
                    <label class="block text-xs text-slate-400 mb-1">Nombre</label>
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="OPERATIVO" class="w-full rounded-lg bg-slate-950 border border-slate-700 px-3 py-2 text-sm text-white">
