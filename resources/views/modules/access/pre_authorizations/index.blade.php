        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mt-6 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100">
                    <h3 class="text-lg font-semibold text-gray-900">Autorizaciones Registradas</h3>
                    <p class="text-sm text-gray-500 mt-0.5">Visitas pre-autorizadas por residentes y anfitriones</p>
                </div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Visitante</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-3">
                                            <p class="text-sm font-medium text-gray-900">{{ $pa->visitor->full_name }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $pa->host->name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $pa->location->name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $pa->scheduled_date->format('d/m/Y') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ring-1
                                        @if($pa->status == 'pending') bg-amber-50 text-amber-700 ring-amber-600/20
                                        @elseif($pa->status == 'used') bg-blue-50 text-blue-700 ring-blue-600/20
                                        @elseif($pa->status == 'cancelled') bg-red-50 text-red-700 ring-red-600/20
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    ...
                                    <p class="mt-2 text-sm text-gray-500">No hay pre-autorizaciones registradas</p>
