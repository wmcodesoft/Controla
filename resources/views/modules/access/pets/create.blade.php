<x-access-layout title="Nueva mascota">
    <div class="max-w-2xl">
        <a href="{{ route('access.pets.index') }}" class="text-sm text-teal-400 hover:text-teal-300">← Mascotas</a>
        <h2 class="text-2xl font-bold text-white mt-2 mb-6">Registrar mascota</h2>
        <form action="{{ route('access.pets.store') }}" method="POST" class="space-y-4 rounded-xl border border-slate-800 bg-slate-900 p-6">
            @csrf
