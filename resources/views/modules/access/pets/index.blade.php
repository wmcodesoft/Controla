<x-access-layout title="Mascotas">
    <div class="space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <a href="{{ route('access.pets.create') }}" class="inline-flex rounded-lg bg-teal-600 px-4 py-2 text-sm font-semibold text-white hover:bg-teal-500">
                Nueva mascota
            </a>
        </div>
        ...
        <form method="GET" class="flex flex-wrap gap-3">
            ...
            <select name="structure_id" class="rounded-lg bg-slate-900 border border-slate-700 px-3 py-2 text-sm text-white">
                ...
                @foreach ($structures as $structure)
                    <option value="{{ $structure->id }}" @selected(request('structure_id') == $structure->id)>{{ $structure->full_path }}</option>
                @endforeach
            </select>
                ...
                <tbody class="divide-y divide-slate-800">
                    @forelse ($pets as $pet)
                        <tr class="hover:bg-slate-800/40">
                            <td class="px-4 py-3">
                                <a href="{{ route('access.pets.show', $pet) }}" class="font-medium text-white hover:text-teal-300">{{ $pet->name }}</a>
                            </td>
                            <td class="px-4 py-3 text-slate-300">{{ $pet->structure?->full_path }}</td>
